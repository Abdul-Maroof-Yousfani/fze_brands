...
//     }
    ...
    public function get_item_by_id(Request $request)
    {
        ...
        // Check if customer has special prices enabled
        $specialPriceMapped = 0;
        if ($customerId) {
            $specialPriceMapped = DB::connection('mysql2')
                ->table('customers')
                ->where('id', $customerId)
                ->value('special_price_mapped');
        }
        ...
        // If special price exists AND is mapped → override product price
        $finalMrp = ($specialPriceMapped == 1 && isset($result->special_mrp) && $result->special_mrp !== '') ? $result->special_mrp : $result->mrp_price;
        $finalSale = ($specialPriceMapped == 1 && isset($result->special_sale) && $result->special_sale !== '') ? $result->special_sale : $result->sale_price;
