    ...
    <div class="dp_sdw">
        <div class="row">
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-center">
                ...
                <h2 class="subHeadingLabelClass">Customer Payment Detail</h2>
            </div>
        ...
        <span id="AllExport">
            ...
            <table class="table table-bordered sf-table-list" id="EmpExitInterviewList{{$table_count}}">
                <thead>
                    <tr>
                        <td colspan="8">
                            <h4 style="margin: 10px 0;"> {{ 'Customer : ' . $row->name }}</h4>
                        </td>
                    </tr>
                    <tr>
                ...
                <tbody id="filterContraVoucherList">
                    ...
                    @endphp
                    <?php
                    $vendor_data = DB::Connection('mysql2')->select('select a.id, a.gi_no, a.gi_date, a.order_no, a.sales_tax, a.sales_tax_further, a.total as header_total, sum(b.amount) as items_amount
                    ...
                    <tr class="text-center">
                        ...
                        $calc_total = $row1->items_amount + $row1->sales_tax + $row1->sales_tax_further + ($Addional->amount ?? 0);
                        $total = (isset($row1->header_total) && $row1->header_total > 0) ? $row1->header_total : $calc_total;
                        ...
                        <td class="text-center">
                            <?php if($paid > 0): ?>
                            <button onclick="showDetailModelOneParamerter('sdc/viewPaymentDetail','<?php echo $row1->id; ?>','View Payment Detail')" type="button" class="btn btn-success btn-xs">Payment Detail</button>
                            <?php else: ?>
